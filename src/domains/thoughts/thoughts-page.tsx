...
const calculateThoughtHeight = (item: ThoughtItem) => {
  let height =
    30 +
    item.richText.reduce((total, segment) => total + segment.plainText.length, 0);
...
function renderRichText(segments: ThoughtRichTextSegment[]): ReactNode[] {
  return segments.map((segment, index) => {
    ...
    if (segment.hashTag) {
      content = <span className="font-medium text-primary">{content}</span>;
    }
        ...
        return (
          ...
          >
            ...
            <div className="relative z-10 space-y-4 p-4 pointer-events-none">
              ...
              <div className={cn("my-4 grid grid-cols-[1fr_auto] items-center gap-3 px-0", item.reactions.length === 0 && "grid-cols-[auto] justify-end")}>
