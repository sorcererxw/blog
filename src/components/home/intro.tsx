...
const getBlockValue = (block: HomePageBlock) =>
  block[block.type] as
    | { rich_text?: unknown; caption?: unknown; url?: string }
    | undefined;
...
const hasVisibleRichText = (richText: NotionRichText[]): boolean =>
  richText.some(
    (segment) =>
      (segment.plain_text ?? segment.text?.content ?? "").trim().length > 0,
  );

const inferImageDimensions = (
  url: string,
        ...
        <li key={item.id} className="text-base text-foreground md:text-lg">
          <div className="space-y-3">
            ...
            {item.children.length > 0 ? (
              <div className="space-y-3">{renderBlocks(item.children)}</div>
            ) : null}
          </div>
        </li>
      ))}
...
function renderBlock(block: HomePageBlock): ReactNode {
  ...
  switch (block.type) {
    ...
    case "heading_3":
      return (
        <>
          <h3 className="font-heading text-xl font-semibold tracking-tight text-foreground sm:text-2xl md:text-3xl">
            {renderRichText(getRichText(block))}
          </h3>
          {renderChildren(block)}
        </>
    ...
    case "paragraph":
      ...
      return (
        <>
          <p>{renderRichText(getRichText(block))}</p>
          {renderChildren(block)}
        </>
      );
    case "quote":
      return (
        <>
          <blockquote>{renderRichText(getRichText(block))}</blockquote>
          {renderChildren(block)}
        </>
      );
    case "callout": {
      ...
      return (
        <Card className={styles.callout}>
          <CardContent className="grid grid-cols-[auto_1fr] gap-4 pt-4">
            <div className="pt-1 text-xl text-muted">{icon ?? "!"}</div>
            <div className="space-y-3 text-base leading-8 text-foreground md:text-lg">
              <div>{renderRichText(getRichText(block))}</div>
              {renderChildren(block)}
            </div>
          </CardContent>
        </Card>
    ...
    case "table": {
      ...
      const bodyRows = rows.slice(table.has_column_header ? 1 : 0);
      ...
      return (
        <div className={styles.tableShell}>
          <RichContentTable aria-label="Home page table">
            <RichContentTableHeader>
              {Array.from({ length: columnCount }).map((_, cellIndex) => (
                <RichContentTableColumn
                  ...
                  key={cellIndex}
                >
                  {table.has_column_header && rows[0]?.cells?.[cellIndex]
                    ? renderRichText(
                        (rows[0].cells[cellIndex].rich_text as
                          | NotionRichText[]
                          | undefined) ?? [],
                      )
                    : `Column ${cellIndex + 1}`}
            ...
            <RichContentTableBody>
              {bodyRows.map((row, rowIndex) => (
                <RichContentTableRow id={rowIndex} key={rowIndex}>
                  {Array.from({ length: columnCount }).map((_, cellIndex) => (
                    <RichContentTableCell key={cellIndex}>
                      {row.cells?.[cellIndex]
                        ? renderRichText(
                            (row.cells[cellIndex].rich_text as
                              | NotionRichText[]
                              | undefined) ?? [],
                          )
                        : null}
...
function renderBlocks(blocks: HomePageBlock[]): ReactNode[] {
  ...
  for (let index = 0; index < blocks.length; ) {
    ...
    nodes.push(<Fragment key={block.id}>{content}</Fragment>);
    index += 1;
  }
  ...
  if (blocks.length === 0) {
    return (
      <section
        ...
        aria-label="Home page content empty state"
      >
        <div className={styles.emptyState}>
          <Card>
            <CardContent>
              <CardTitle>Home</CardTitle>
              <CardDescription>The home page content is empty.</CardDescription>
            </CardContent>
          </Card>
        </div>
      </section>
