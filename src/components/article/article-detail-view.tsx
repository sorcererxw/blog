...
import {
  ...
  RichContentTableRow,
} from "@/components/rich-content/rich-content-table";
...
const renderCaption = (
  ...
  return (
    <figcaption>{renderRichText(richText, fallback ?? undefined)}</figcaption>
  );
};
...
function renderToc(nodes: ArticleDetailTocNode[]): ReactNode {
  return nodes.map((node) => (
    <Fragment key={node.id}>
      ...
      {node.nodes.length > 0 ? (
        <div className={styles.tocChildren}>{renderToc(node.nodes)}</div>
      ) : null}
    </Fragment>
  ));
}
...
function renderListItems(
  items: Extract<ArticleDetailBlock, { kind: "list-item" }>[],
  ordered: boolean,
) {
...
function renderTableBlock(
  block: Extract<ArticleDetailBlock, { kind: "table" }>,
) {
  const headerRowIndex = block.rows.findIndex((row) =>
    row.cells.some((cell) => cell.header),
  );
  const headerRow =
    headerRowIndex >= 0 ? block.rows[headerRowIndex] : undefined;
  const bodyRows = headerRow
    ? block.rows.filter((_, index) => index !== headerRowIndex)
    : block.rows;
  const columnCount = Math.max(...block.rows.map((row) => row.cells.length), 0);
  ...
  return (
    <div className={styles.tableShell}>
      <RichContentTable aria-label="Article table">
        <RichContentTableHeader>
          {Array.from({ length: columnCount }).map((_, cellIndex) => (
            <RichContentTableColumn
              id={`column-${cellIndex}`}
              isRowHeader={cellIndex === 0}
              key={cellIndex}
            >
              {headerRow?.cells[cellIndex]
                ? renderRichText(headerRow.cells[cellIndex].richText)
                : `Column ${cellIndex + 1}`}
            </RichContentTableColumn>
          ))}
        </RichContentTableHeader>
        <RichContentTableBody>
          {bodyRows.map((row, rowIndex) => (
            <RichContentTableRow id={rowIndex} key={rowIndex}>
              {Array.from({ length: columnCount }).map((_, cellIndex) => (
                <RichContentTableCell
                ...
                >
                  {row.cells[cellIndex]
                    ? renderRichText(row.cells[cellIndex].richText)
                    : null}
                </RichContentTableCell>
              ))}
            </RichContentTableRow>
            ...
            className={cn(
              block.level === 1
                ...
                : block.level === 2
                  ...
                  : "font-heading text-xl font-semibold tracking-tight text-foreground md:text-2xl",
            )}
            id={block.id}
          >
          ...
          <div className="grid gap-2">
            ...
            {block.highlightedHtml ? (
              ...
              <pre className="overflow-x-auto text-sm text-foreground">
                <code className="whitespace-pre-wrap font-code">
                  {block.text}
                </code>
              </pre>
            )}
          ...
          >
            <Card className={styles.bookmark}>
              ...
              <CardContent className="space-y-2">
                ...
                <p className="break-all text-sm text-muted">{block.url}</p>
              </CardContent>
            </Card>
          </NextLink>
          {renderCaption(block.captionRichText, block.caption)}
      ...
      return (
        <Card className={cn(styles.callout, colorToClassName(block.color))}>
          <CardContent className="grid grid-cols-[auto_1fr] gap-4 pt-4">
            <div className="pt-1 text-xl text-muted">{block.emoji ?? "!"}</div>
  ...
  blocks.forEach((block, index) => {
    ...
    renderedBlocks.push(
      <div
        ...
        className={cn(
          block.kind === "image" || block.kind === "bookmark"
            ? styles.mediaBlock
            : "",
        )}
      >
      ...
      <div className={styles.page}>
        <header className={styles.hero}>
          <div
            ...
            aria-hidden={article.cover ? undefined : true}
          >
            {article.cover ? (
              <ResponsiveRemoteImage
                alt={article.title}
              ...
              <span className={styles.authorBadge} aria-hidden="true">
                {iconLabel ? (
                  ...
                  <span
                    ...
                    style={{
                      backgroundImage: `url(${buildCloudflareImageUrl(article.icon.value, "icon")})`,
                    }}
                  />
                ) : (
          ...
          <section aria-label="Article content" className={styles.reading}>
            {article.blocks.length > 0 ? (
              ...
              <p className="publication-note">
                This article has no rendered content blocks yet.
              </p>
            )}
          </section>
