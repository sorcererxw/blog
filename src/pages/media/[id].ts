...
const redirectToSource = (sourceUrl: string) =>
  Response.redirect(sourceUrl, 302);

async function getCloudflareEnv(
  locals: unknown,
): Promise<Partial<CloudflareEnv> | null> {
  if (locals && typeof locals === "object") {
    const testEnv = (locals as { __testEnv?: Partial<CloudflareEnv> }).__testEnv;
    if (testEnv) {
      return testEnv;
    }
  }
  ...
  try {
    ...
    return workers.env as Partial<CloudflareEnv>;
  } catch {
    return null;
  }
}
...
export const GET: APIRoute = async ({ params, request, locals }) => {
  ...
  const runtimeEnv = await getCloudflareEnv(locals);
